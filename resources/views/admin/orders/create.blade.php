            ...
            <div class="kt-portlet__head">
                <div class="kt-portlet__head-label">
                    <h3 class="kt-portlet__head-title">
                        {{ __('dashboard.add_new_order') }}
                    </h3>
                </div>
            </div>
            <div class="container p-5">
                <form action="{{ route('systemOrders.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                            ...
                            <select name="product_id" id="product"
                                ...
                                @foreach ($products as $product)
                                    <option data-url="{{ route('order_product', $product->id) }}"
                                        value="{{ $product->id }}" @if ($product->id == old('product')) selected @endif>
                                        {{ $product->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    {{-- <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            <select name="status" >
                                <option value="new">{{ __('home.new') }}</option>
                                <option value="processing">{{ __('home.processing') }}</option>
                                <option value="in-delivery">{{ __('home.in_delivery') }}</option>
                                <option value="completed">{{ __('home.completed') }}</option>
                            </select>
